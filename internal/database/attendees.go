...
func (m *AttendeeModel) Insert(attendee *Attendee) (*Attendee, error) {
	...
	query := "INSERT INTO attendess (event_id, user_id) VALUES ($1, $2) RETURNING id"
...
func (m *AttendeeModel) GetByEventAndAttendee(eventId, userId int) (*Attendee, error) {
	...
	query := "SELECT * FROM attendess WHERE event_id = $1 AND user_id = $2"

	var attendee Attendee

	err := m.DB.QueryRowContext(ctx, query, attendee.EventId, attendee.UserId).Scan(&attendee.Id, &attendee.EventId, &attendee.UserId)
	if err != nil {
